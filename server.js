...
// capture info for legal moves, check indicator, resign/draw/new game, load setup code
...
function pseudoMoves(state, fromX, fromY) {
  ...
  if (p.type === "p") {
    ...
    for (const dx of [-1, 1]) {
      ...
      if (t && t.color !== p.color) add(cx, cy, { promotion: (cy === lastRank) });
    }
    ...
    if (enPassant) {
      for (const dx of [-1, 1]) {
        const tx = fromX + dx;
        const ty = fromY + dir;
        if (tx === enPassant.x && ty === enPassant.y) add(tx, ty, { enPassant: true });
      }
    }
...
function applyMove(state, mv) {
  ...
  if (p.type === "p" && mv.promotion) {
    const promo = mv.promoType || "q"; // q r b n
...
function legalMovesFrom(state, fromX, fromY) {
  ...
  for (const mv of pseudo) {
    const mvCheck = { ...mv };
    if (mvCheck.promotion && !mvCheck.promoType) mvCheck.promoType = "q";
...
function serializeBoard(board, N) {
  ...
  for (let y = 0; y < N; y++) for (let x = 0; x < N; x++) {
    const p = board[y][x];
    if (p) pieces.push({ x, y, color: p.color, type: p.type });
  }
  return pieces;
}
...
function attachSeat(room, side, socket, token) {
  ...
  socket.data.side = side;       // "white" | "black" | "spectator"
...
function clearSeatSocket(room, side, socketId) {
  ...
  seat.timer = setTimeout(() => {
    ...
    if (s.socketId) return; // reclaimed
    s.token = null;
    s.timer = null;
...
function validateSetup(board, N) {
  ...
  if (!wk || !bk) return { ok: false, msg: "You must place exactly 1 white king and 1 black king." };

  let wkc = 0, bkc = 0;
  for (let y = 0; y < N; y++) for (let x = 0; x < N; x++) {
    const p = board[y][x];
    if (p && p.type === "k") (p.color === "w" ? wkc++ : bkc++);
  }
  if (wkc !== 1 || bkc !== 1) return { ok: false, msg: "Setup must have exactly 1 king per side." };
...
function broadcast(room) {
  ...
  io.to(room.code).emit("roomState", {
    ...
    // ✅ new fields
    drawOfferFrom: room.drawOfferFrom, // "w" | "b" | null
    check: chk.check,                 // {w:boolean,b:boolean}
    kingPos: chk.kingPos,             // {w:{x,y}|null,b:{x,y}|null}
  });
}
...
io.on("connection", (socket) => {
  socket.on("createRoom", ({ sessionToken, hostColor, boardSize } = {}) => {
    ...
    socket.emit("joinedRoom", { roomCode: code, side: hostSide, isHost: true, sessionToken: token });
    broadcast(room);
  });
  ...
  socket.on("rejoinRoom", ({ roomCode, sessionToken } = {}) => {
    ...
    const room = rooms.get(code);
    if (!room || !token) return socket.emit("joinError", { message: "Could not rejoin room." });

    const side = assignOrReclaim(room, socket, token);

    socket.emit("joinedRoom", { roomCode: code, side, isHost: token === room.hostToken, sessionToken: token });
    broadcast(room);
  });

  // host-only: change board size during SETUP (resets)
  ...
  // ✅ host-only: load setup (sets size + places pieces)
  ...
  socket.on("startGame", () => {
    ...
    if (!v.ok) return socket.emit("actionError", { message: v.msg });
  ...
  socket.on("declineDraw", () => {
    ...
    // offerer can "cancel"; opponent can "decline"
    room.drawOfferFrom = null;
    broadcast(room);
  });

  // ✅ new game (host-only) — resets to setup on same board size
  socket.on("newGame", () => {
    const room = rooms.get(socket.data.roomCode);
    if (!room) return;
    if (!socket.data.token || socket.data.token !== room.hostToken) return;
  ...
  socket.on("requestMoves", ({ x, y }) => {
    ...
    const st = room.state;
    if (st.phase !== "play") return socket.emit("legalMoves", { from: { x, y }, moves: [] });

    const side = socket.data.side;
    const myC = side === "white" ? "w" : side === "black" ? "b" : null;
    if (!myC || myC !== st.turn) return socket.emit("legalMoves", { from: { x, y }, moves: [] });

    if (!inBounds(x, y, st.N)) return socket.emit("legalMoves", { from: { x, y }, moves: [] });

    const p = pieceAt(st.board, x, y);
    if (!p || p.color !== myC) return socket.emit("legalMoves", { from: { x, y }, moves: [] });
    ...
    const moves = legal.map((m) => {
      ...
      return { x: m.toX, y: m.toY, promotion: !!m.promotion, capture };
    });
  ...
  socket.on("move", ({ from, to, promoType }) => {
    ...
    if (!found) return socket.emit("actionError", { message: "Illegal move." });

    const mv = { ...found, promoType: found.promotion ? (promoType || "q") : undefined };
    const next = applyMove(st, mv);
    if (!next) return;

    room.state = next;
    room.drawOfferFrom = null; // clear draw offer on any move
    endIfNeeded(room);
    broadcast(room);
  });
...
server.listen(PORT, () => console.log(`Chess server running at http://localhost:${PORT}`));